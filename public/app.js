          ...
          <div class="flex items-center justify-end gap-1.5 opacity-40 group-hover:opacity-100 transition-all duration-300">
            ...
            <div class="relative group/menu mt-1">
              ...
              <div class="absolute right-0 top-full mt-2 w-44 bg-surface-card border border-surface-border rounded-xl shadow-xl opacity-0 invisible group-hover/menu:opacity-100 group-hover/menu:visible transition-all z-50 p-1 origin-top-right transform scale-95 group-hover/menu:scale-100">
                ...
                <button onclick="openMergeModal('${escapedPath}', '${escapedWs}', ${i})" class="w-full flex items-center gap-2 px-3 py-2 text-[13px] font-medium text-violet-400 hover:text-violet-300 hover:bg-violet-500/10 rounded-lg transition-colors border-t border-surface-border mt-1 pt-2">
                  <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4"/></svg> Merge
                </button>
...
async function refreshSingleRepo(repoPath, wsPath, i) {
  ...
  // Add a subtle loading overlay to the row
  ...
  try {
    ...
    showToast(`Refreshed: ${repoPath.split(/[/\\\\]/).pop()}`, 'success');
  } catch (err) {
...
// ===== MERGE MODAL =====
let mergeModalContext = { path: '', workspace: '', rowIndex: -1 };
let mergeSelectedBranch = '';
let mergeAllBranches = [];
let mergeLocalBranches = [];
let mergeRemoteBranches = [];
let mergeCurrentBranch = '';

window.openMergeModal = async function (repoPath, wsPath, rowIndex) {
  mergeModalContext = { path: repoPath, workspace: wsPath, rowIndex };
  mergeSelectedBranch = '';

  // Find current branch from repo data
  const repo = currentRepos.find(r => r.path === repoPath);
  const repoName = repo ? repo.name : repoPath.replace(/\\/g, '/').split('/').pop();
  mergeCurrentBranch = repo ? repo.branch : 'unknown';

  // Set repo context in modal
  document.getElementById('merge-repo-name').textContent = repoName;
  document.getElementById('merge-current-branch').textContent = mergeCurrentBranch;

  // Show modal with loading state
  const modal = document.getElementById('merge-modal');
  document.getElementById('merge-modal-loading').classList.remove('hidden');
  document.getElementById('merge-modal-content').classList.add('hidden');
  modal.classList.remove('hidden');
  requestAnimationFrame(() => {
    modal.classList.remove('opacity-0');
    const inner = modal.querySelector('.scale-95');
  ...
  try {
    ...
    // Filter out current branch — can't merge current into itself
    mergeLocalBranches = allBranches.filter(b => !b.startsWith('remotes/') && b !== mergeCurrentBranch);
    mergeRemoteBranches = allBranches.filter(b => b.startsWith('remotes/') && !b.includes('HEAD'));

    document.getElementById('merge-count-local').textContent = `(${mergeLocalBranches.length})`;
    document.getElementById('merge-count-remote').textContent = `(${mergeRemoteBranches.length})`;

    document.getElementById('merge-modal-loading').classList.add('hidden');
    document.getElementById('merge-modal-content').classList.remove('hidden');

    // Default to local tab
    switchMergeTab('local');
    document.getElementById('merge-branch-filter').value = '';
  } catch (err) {
    showToast(`Lỗi tải branch: ${err.message}`, 'error');
    closeMergeModal();
  }
};
...
window.switchMergeTab = function (tab) {
  const localTab = document.getElementById('merge-tab-local');
  const remoteTab = document.getElementById('merge-tab-remote');
  if (tab === 'local') {
    localTab.className = localTab.className.replace(/text-slate-400 hover:text-slate-300 hover:bg-white\/5/g, '').trim();
    localTab.classList.add('bg-violet-500/20', 'text-violet-400', 'border', 'border-violet-500/30');
    remoteTab.className = remoteTab.className.replace(/bg-violet-500\/20|text-violet-400|border-violet-500\/30/g, '').trim();
    remoteTab.classList.add('text-slate-400', 'hover:text-slate-300', 'hover:bg-white/5');
    renderMergeBranchList(mergeLocalBranches, false);
  } else {
    remoteTab.className = remoteTab.className.replace(/text-slate-400 hover:text-slate-300 hover:bg-white\/5/g, '').trim();
    remoteTab.classList.add('bg-violet-500/20', 'text-violet-400', 'border', 'border-violet-500/30');
    localTab.className = localTab.className.replace(/bg-violet-500\/20|text-violet-400|border-violet-500\/30/g, '').trim();
    localTab.classList.add('text-slate-400', 'hover:text-slate-300', 'hover:bg-white/5');
    renderMergeBranchList(mergeRemoteBranches, true);
  }
  mergeSelectedBranch = '';
  updateMergeButton();
};

function renderMergeBranchList(branches, isRemote) {
  const list = document.getElementById('merge-branch-list');
  if (branches.length === 0) {
    list.innerHTML = '<div class="text-xs text-slate-500 text-center py-4 italic">Không có branch nào.</div>';
    return;
  }
  list.innerHTML = branches.map(b => {
    const display = isRemote ? b.replace(/^remotes\//, '') : b;
    const selected = mergeSelectedBranch === b;
    return `<button onclick="selectMergeBranch('${b.replace(/'/g, "\\'")}')"
          class="w-full text-left px-3 py-2 rounded-lg text-[13px] font-medium transition-all duration-150 flex items-center gap-2
          ${selected ? 'bg-violet-500/20 text-violet-300 border border-violet-500/30' : 'text-slate-300 hover:bg-white/5 hover:text-white border border-transparent'}"
          data-branch-name="${display.toLowerCase()}">
          ${isRemote ? '<svg class="w-3.5 h-3.5 flex-shrink-0 opacity-60" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 15a4 4 0 004 4h9a5 5 0 10-.1-9.999 5.002 5.002 0 10-9.78 2.096A4.001 4.001 0 003 15z"/></svg>' : ''}
          <span class="truncate">${display}</span>
        </button>`;
  }).join('');
}

window.selectMergeBranch = function (branch) {
  mergeSelectedBranch = branch;
  // Re-highlight the selected item
  document.querySelectorAll('#merge-branch-list button').forEach(btn => {
    const isCurrent = btn.getAttribute('onclick')?.includes(branch.replace(/'/g, "\\'"));
    if (isCurrent) {
      btn.className = btn.className.replace(/text-slate-300 hover:bg-white\/5 hover:text-white border-transparent/g, '');
      btn.classList.add('bg-violet-500/20', 'text-violet-300', 'border-violet-500/30');
      btn.classList.remove('border-transparent');
    } else {
      btn.classList.remove('bg-violet-500/20', 'text-violet-300', 'border-violet-500/30');
...
function updateMergeButton() {
  ...
  btn.disabled = !mergeSelectedBranch;
}

window.filterMergeBranchList = function (query) {
  const lowerQuery = query.toLowerCase();
  document.querySelectorAll('#merge-branch-list button').forEach(btn => {
    const name = btn.getAttribute('data-branch-name') || '';
    btn.style.display = name.includes(lowerQuery) ? 'flex' : 'none';
  });
};

window.executeMerge = async function () {
  if (!mergeSelectedBranch) {
    showToast('Vui lòng chọn một branch để merge!', 'warning');
    return;
  }

  const { path: repoPath, workspace, rowIndex } = mergeModalContext;
  const display = mergeSelectedBranch.startsWith('remotes/') ? mergeSelectedBranch.replace('remotes/', '') : mergeSelectedBranch;
  const repoName = repoPath.replace(/\\/g, '/').split('/').pop();
  const noFf = document.getElementById('merge-noff').checked;

  const isConfirmed = await customConfirm(`[${repoName}] Merge "${display}" vào "${mergeCurrentBranch}"?\n\nRepo: ${repoPath}`);
  if (!isConfirmed) return;

  const btn = document.getElementById('btn-execute-merge');
  btn.disabled = true;
  const originalHTML = btn.innerHTML;
  btn.innerHTML = `<svg class="w-4 h-4 spin" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/></svg> Merging...`;

  try {
    const res = await fetch('/api/repo/merge', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ path: repoPath, workspace, branch: mergeSelectedBranch, noFf }),
    });
    const data = await res.json();
    if (!res.ok) throw new Error(data.error || 'Merge thất bại');

    closeMergeModal();
    showToast(`Merge thành công trên <b>${repoName}</b>!`, 'success');

    // Update local data & re-render row
    if (data.repo) {
      const originalIndex = currentRepos.findIndex(r => r.path === repoPath);
      if (originalIndex !== -1) {
        currentRepos[originalIndex] = data.repo;
      ...
      const tr = document.getElementById('repo-row-' + rowIndex);
      if (tr) tr.innerHTML = renderRepoRowInner(data.repo, rowIndex);
    }
  } catch (err) {
    showToast(`Merge thất bại trên <b>${repoName}</b>:\n${err.message}`, 'error');
  } finally {
    btn.disabled = false;
    btn.innerHTML = originalHTML;
  }
